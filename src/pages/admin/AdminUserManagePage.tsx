...
export default function AdminUserManagePage() {
    const { users, setUsers } =
        useOutletContext<AdminUserOutletContext>();
    ...
    const filtered = users.filter((u) => {
      ...
      if (!kw) return true;
      const hay = `${u.nickname ?? ''} ${u.username ?? ''}`.toLowerCase();
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex shrink-0 flex-wrap items-center gap-2 text-xs text-slate-500">
                                    <span className="font-bold">가입 일자</span> { formatDate(row.createdAt) }
                                    { row.status === 'DELETED' &&  <div><span className="font-bold"> · 탈퇴 일자</span> { row.deletedAt && formatDate(row.deletedAt) }</div> }
                                </div>
                                <div className="flex gap-1.5 shrink-0 items-center">
                                    <Button type="button" variant="outline" size="sm" className="h-8 text-xs">
                                        상세
                                    </Button>
                                    <Button type="button" variant="admin" size="sm" className="h-8 text-xs"
                                            onClick={() =>
